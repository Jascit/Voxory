...
namespace containers {
  namespace detail {
    ...
    public:
      ...
      CONSTEXPR ConstStaticArrayIterator(pointer ptr) noexcept : ptr_(ptr) {}
      ...
      CONSTEXPR ConstStaticArrayIterator& operator+=(const difference_type off) noexcept {
        ptr_ += off;
        return *this;
      }

      NODISCARD CONSTEXPR ConstStaticArrayIterator operator+(const difference_type off) const noexcept {
        ConstStaticArrayIterator tmp = *this;
        tmp += off;
        return tmp;
      }
    ...
    // Non-const iterator: inherits const iterator but exposes mutable reference/pointer
    template<typename Traits>
    ...
    public:
      ...
      CONSTEXPR StaticArrayIterator(pointer ptr) noexcept : base_type(ptr) {}

      NODISCARD CONSTEXPR reference operator*() const noexcept {
        // base_type::operator*() returns const_reference, cast away const for non-const iterator
      ...
      CONSTEXPR StaticArrayIterator& operator+=(const difference_type off) noexcept {
      ...
      NODISCARD CONSTEXPR StaticArrayIterator operator+(const difference_type off) const noexcept {
        StaticArrayIterator tmp = *this;
        tmp += off;
        return tmp;
      }
      ...
      NODISCARD CONSTEXPR reference operator[](const difference_type off) const noexcept {
        // cast away const from base operator[]
