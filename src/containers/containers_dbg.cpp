...
container_base_dbg::container_base_dbg(container_base_dbg&& o) noexcept(noexcept(std::declval<list&>() = std::move(std::declval<list&>()))) : list_{} {
   o.lock();
   list_ = std::move(o.list_);
   o.ver_.fetch_add(1, std::memory_order::memory_order_release);
   o.unlock();
...
void iterator_base_dbg::copy_proxy(const iterator_base_dbg& o) {
  release();
  container_ = o.container_;
  register_to_container(container_);
}

void iterator_base_dbg::release() {
  if (!proxy_) return;
